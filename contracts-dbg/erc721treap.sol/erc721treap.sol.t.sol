...
contract treap {
    ...
    function insert(address _whitelist_user) public{
        ...
        root[0].right = merge(0, root.length-1);
    }
    ...
    function print() public view returns (uint256[] memory) {
        uint256[] memory result = new uint256[](root.length * 2);
        uint256 index = 0;
        for (uint256 i = 0; i < root.length; i++) {
            result[index++] = root[i].left;
            result[index++] = root[i].right;
        }
        return result;
    }
...
contract FanartNFT is ERC721 {
   ...
   address[] public whitelist = [
    ...
    string _name = "TXT BEOMGYU";
    string _symbol = "TXT Bear";
