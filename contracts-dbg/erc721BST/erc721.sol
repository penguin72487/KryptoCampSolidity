...
contract Treap {
    ...
    function insert(address _whitelist_user) public {
        rootIndex = _insert(rootIndex, Node(uint256(keccak256(abi.encodePacked(_whitelist_user))),random(),0,0));
    }
    ...
    function verify(address _whitelist_user) public view returns (bool) {
        return _verify(rootIndex, uint256(keccak256(abi.encodePacked(_whitelist_user))));
    }
