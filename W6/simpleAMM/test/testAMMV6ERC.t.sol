...
contract TestSimpleAMMTest is Test {
    ...
    function setUp() external {
        ...
        vm.prank(user1);
        token0.approve(address(ammInstance), 4000 * 10**18);
        token1.approve(address(ammInstance), 4000 * 10**18);
        vm.prank(user2);
        token0.approve(address(ammInstance), 4000 * 10**18);
        token1.approve(address(ammInstance), 4000 * 10**18);
    }
    ...
    function testAddLiquidity() external {
        ...
        vm.prank(user2);
        ammInstance.addLiquidity{value: 20 ether}(2000 * 10**18, 2000 * 10**18);
