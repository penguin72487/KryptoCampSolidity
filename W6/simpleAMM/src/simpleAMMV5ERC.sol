...
//         require(msg.value == _amountIn, "token0 amount mismatch");
...
//     //     _update(address(this).balance, token.balanceOf(address(this)));
//     // }
//     function swapTokenFortoken0(uint256 _amountIn) public returns (uint256 amountOut) {
//         uint256 amountInWithFee = (_amountIn * 997) / 1000;
...
//     }
//     // function _swapTokenFortoken0WithOutFee(uint256 _amountIn) internal returns (uint256 amountOut) {
...
//         token0.transferFrom(msg.sender, address(this), _amount1);
...
//                 // Swap the excess tokens for token0 without fee
//                 if (_amount1 > requiredAmount1) {
//                     uint256 excessAmount1 = _amount1 - requiredAmount1;
//                     _amount0 += swapTokenFortoken0(excessAmount1);

//                 // Swap the excess token0 for tokens without fee
//                 } else {
...
//         require(reserve0 > 0 && reserve1 > 0 && _amount>0, "Invalid token0 reserves");
//         uint256 amountInWithFee=(_amount * 997) / 1000;
...
//         uint256 noSlippagePrice = gettoken1Price();
